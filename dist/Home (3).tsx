...
{bannersLoaded && activeBanners.length > 0 && (
  <section className="bg-slate-100 py-4 w-full" data-banner-type="carousel">
    <div className="max-w-7xl mx-auto px-4 lg:px-6 w-full">
      ...
      {/* Multi-buy Deals Section */}
      {multiBuyProducts.length > 0 && (
        <section className="py-6 md:py-10 bg-yellow-50 border-y border-yellow-200">
          <div className="max-w-7xl mx-auto px-4 lg:px-6 w-full">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <div>
                <h2 className="text-xl md:text-2xl font-bold text-[#2D3748]">Multi-buy Deals</h2>
                <p className="text-sm text-slate-600 mt-0.5">Buy more, save more — bulk pricing on these products</p>
              </div>
              <Link
                to="/products?multibuy=true"
                className="flex items-center gap-2 px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-slate-900 rounded-lg text-sm font-bold transition-colors"
              >
                View All Multi-buy Deals
                <ArrowRight className="size-4" />
              </Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {multiBuyProducts.map((product: any, index: number) => (
                <Link
                  key={product.id}
                  to={`/products/${product.id}`}
                  className="group bg-white rounded-xl border border-yellow-200 hover:border-yellow-400 hover:shadow-md transition-all overflow-hidden flex flex-col"
                >
                  <div className="relative aspect-square bg-slate-50 overflow-hidden">
                    ...
                    <div className="absolute top-2 left-2 bg-yellow-400 text-slate-900 text-xs font-bold px-2 py-0.5 rounded-full">
                      Multi-buy
                    </div>
                  </div>
                  <div className="p-3 flex flex-col gap-1 flex-1">
                    <p className="text-sm font-semibold text-slate-800 leading-snug line-clamp-2">{product.name}</p>
                    {product.code && <p className="text-xs text-slate-400">Code: {product.code}</p>}
                    <div className="mt-auto pt-2">
                      <p className="text-base font-bold text-[#E31837]">
                        ${((product.sellingPrice || product.salePrice || product.price) ?? 0).toFixed(2)}
                      </p>
                      {product.multiBuyOptions && product.multiBuyOptions[1] && (
                        <p className="text-xs text-green-700 font-semibold">
                          Buy {product.multiBuyOptions[1].quantity}+ @ ${product.multiBuyOptions[1].price?.toFixed(2)} each
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <section className="py-16 md:py-20 bg-gradient-to-br from-[#2D3748] via-[#424B54] to-[#2D3748]">
        <div className="max-w-5xl mx-auto px-4 lg:px-6 w-full text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <a href="tel:1800151654">
              <Button size="lg" className="bg-[#E31837] hover:bg-[#C41230] text-white text-lg px-10 py-7 h-auto shadow-xl w-full sm:w-auto">
                <Phone className="size-6 mr-2" />
                Call 1800 151 654
              </Button>
            </a>
            <Link to="/products">
              <Button size="lg" variant="outline" className="bg-white hover:bg-slate-100 text-[#2D3748] border-0 text-lg px-10 py-7 h-auto shadow-xl w-full sm:w-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 text-white">
            <a href="tel:1800151654" className="block hover:opacity-80 transition-opacity">
              <Phone className="size-8 mx-auto mb-3 text-[#E31837]" />
              <p className="font-semibold text-lg mb-1">Call Us</p>
              <p className="text-[#E31837] font-bold text-lg">1800 151 654</p>
              <p className="text-slate-400 text-sm mt-0.5">Tap to call now</p>
            </a>
            <a href="mailto:admin@costplus100.com.au" className="block hover:opacity-80 transition-opacity">
              <Mail className="size-8 mx-auto mb-3 text-[#E31837]" />
              <p className="font-semibold text-lg mb-1">Email Us</p>
              <p className="text-[#E31837] font-medium text-sm break-all">admin@costplus100.com.au</p>
              <p className="text-slate-400 text-sm mt-0.5">Get a detailed quote</p>
            </a>
            <Link to="/products" className="block hover:opacity-80 transition-opacity">
              <Search className="size-8 mx-auto mb-3 text-[#E31837]" />
              <p className="font-semibold text-lg mb-1">Browse First</p>
              <p className="text-slate-300 text-sm">13,000+ products</p>
              <p className="text-slate-400 text-sm mt-0.5">Explore our range</p>
            </Link>
          </div>
        </div>
      </section>
