...
while (true) {
  ...
  // Match any product that is from Simco (by category or importSource or brand)
  const simco = data.filter(row => {
    const v = row.value;
    return (
      v?.importSource === 'simco' ||
      v?.category === 'Simco Equipment' ||
      String(v?.categoryLevel1).toLowerCase().includes('simco') ||
  ...
  if (simco.length > 0) {
    const upserts = simco.map(row => {
      ...
      const alreadyDiscounted = row.value.wasPrice && row.value.wasPrice !== originalPrice;
      if (alreadyDiscounted) return null; // skip already discounted
